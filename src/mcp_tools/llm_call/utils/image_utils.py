...
def process_image_input(
    image_input: str, image_directory: str, max_size_kb: int = 500
) -> Dict[str, Any]:
    """
    ...
    if image_input.startswith("http"):  # External URL
        return {"type": "image_url", "url": image_input}
    elif image_input.startswith("data:image"):  # Base64 string
        compressed_base64_image = decode_base64_image(
            image_input, image_directory, max_size_kb
        )
        return {"type": "image_url", "url": compressed_base64_image}
        ...
        return {"type": "image_url", "url": base64_image}
...
def compress_image(
    ...
    compressed_file_path = os.path.join(
        image_directory, f"{os.path.splitext(original_file_name)[0]}_compressed.jpg"
    )
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            img.save(
                compressed_file_path, format=img_format, quality=quality, optimize=True
            )
            ...
            if attempt < max_attempts - 1:  # Avoid resizing on the last attempt
                ...
                logger.info(
                    f"Resizing image to {width}x{height} for attempt {attempt + 1}."
                )

            quality = max(10, quality - 10)  # Reduce quality for further compression

        logger.warning(
            f"Could not compress {image_path} under {max_size_kb}KB after {max_attempts} attempts."
        )
        return image_path
...
def decode_base64_image(
    base64_image: str, image_directory: str, max_size_kb: int
) -> str:
    """
    ...
    try:
        ...
        if original_size_kb <= max_size_kb:
            logger.info(
                f"Base64 image is already within the size threshold ({original_size_kb}KB)."
            )
            return base64_image

        # Compress the image if it exceeds the size threshold
        compressed_image_path = compress_image(
            temp_image_path, image_directory, max_size_kb
        )
        ...
        logger.info(
            f"Compressed Base64 image to meet the size threshold ({max_size_kb}KB)."
        )
