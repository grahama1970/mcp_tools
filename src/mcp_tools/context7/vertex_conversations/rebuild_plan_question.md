General Workflow and Assumptions:

Entry Point: What is the primary entry point for the overall system? Is it the CLI (cli.py), the FastAPI server (api.py), or is there a separate orchestrator script? This affects how we structure the initial tasks.
> the cli will be for local LLM agent use. The fast api endpoints with sse-starlette will be used to create an MCP container so the project is more portable/flexible/modular. The fastapi endpoints are a mirror of the cli commands. Fastapi is external, cli commmand (by the package typer) are local
You will need to create a dockerfile and docker-compose.yml for this which launches arangodb(the main database), redis (the caching database for litellm) and the fastapi/MCP container

Data Flow: After the sparse checkout, is the data processed sequentially (file discovery -> extraction -> embedding -> storage) or are there opportunities for parallel processing? Understanding this will influence the architecture of the main pipeline.
> the embedding (by litellm) will be concurrent in batches of 50 using asyncio task gather and as_completed with a pbar tqdm. each as_completed will move the progress bar
> yes, the user (or the agent) provides a list of repos and each repo is sequentially proceed (sparse download -> file discover -> extraction into a well formatted json object to be saveed to file -> embedding -> storage )

Incremental Processing: Is the system designed to re-process entire repositories on each run, or should it be able to handle incremental updates (i.e., only process files that have changed since the last run)? If incremental processing is required, we need to consider how to track file changes.
> no, if the repo has already been proceed, do not re-process unless the user/agent provided a --force parameter or an --age parameter 

Configuration Management Strategy: Is it preferable to rely primarily on environment variables, or should a configuration file (e.g., a config.yaml or config.json) be used in conjunction with environment variables? This influences how we implement config.py.
> enviromental variable for sensitive info like API keys. a config file (yaml or a dictionary is fine) for non sensitive data

Specific Task-Related Questions:


Phase 2 (Sparse Checkout):

How should errors from the Perplexity MCP tool be handled? Should they be considered fatal errors, or should the system attempt to proceed with a default repository link or skip the repository entirely?
> the human/agent provides a list of repos to process. If a repo link fails, the ask-perplexity MCP tool should be used (once) to try to find the correct repo link. If that repo link fails, log the error and continue with processing the next repo

Are there any specific requirements for validating the structure of the downloaded directory? (e.g., checking for the existence of certain subdirectories or files).
> we need to check that there are md, mdx, rst, text, and ipynb files beyond the README. you can grep or os.listdir to recurse through to find files like: https://github.com/fastapi/fastapi/blob/master/docs/en/docs/async.md

Phase 3 (File Type Handling and Extraction):

Are there any specific libraries or tools that should be used for extracting text from .rst files?
> use docutils

Are there any specific requirements for handling binary files or other non-text files that might be encountered during file discovery?
> exclude all binary files and all text file over 50k

Phase 4 (Code Block and Description Extraction):

Are there any specific patterns or delimiters that should be used to identify code blocks and descriptions within each file type?
> for code, you are looking for triple backticks, for descriptions look for paragraph text above the triple backticks and a section title marked by checkbox. use markdown-it-py to help you

Should the system attempt to normalize or clean the extracted code blocks and descriptions (e.g., remove leading/trailing whitespace, correct indentation)?
> markdown is generally well formatted. Remember you are converting the markdown into well formatted json. So you will want to remove whitespace and format  with fields title, description, code, embedding, embedding_metadata, date extracted and similar. well formatted example here  
{
  "settings": {
    "title": "Apache ActiveMQ",
    "project": "activemq",
    "docsRepoUrl": "https://github.com/apache/activemq-website",
    "folders": [],
    "excludeFolders": [
      "*archive*",
      "*archived*",
  ...
  "version": {
    ...
    "state": "finalized",
    "parseDuration": 705887,
    "totalTokens": 144370,
    "totalSnippets": 1283,
    "averageTokens": 112.52533125487139,
    "errorCount": 1,
    "validationDate": "2025-03-19T23:41:30.826Z"
  }
}

Phase 5 (Embedding Generation):

Are there any specific requirements for the format or dimensionality of the embedding vectors?
use the models avaiable within litellm: https://docs.litellm.ai/docs/embedding/supported_embedding
deafaul to the text-embedding-ada-002 from openai

Are there any constraints on which LiteLLM models can be used for embedding generation?
How should rate limits be handled when using LiteLLM for embedding generation?
> use a tenacity decorator with exponential backoff for each litellm call. Note in a asycio task gather, you need a singular litellm call function using acompletiont that uses the tenacity decorator. Yu can can't put the decorate on a complex function with logic
Phase 6 (Database Storage):

Are there any specific indexes or constraints that should be defined on the ArangoDB collection to optimize query performance?
> we will be creating a view with a vector 
...
Are there any specific data validation requirements that should be enforced before storing data in the ArangoDB database?
> the json file must be well formatted and must contain code and a description and and embedding field. You can use modern Pyndantic class to help you with data validation

Is there a strategy for versioning the data stored in ArangoDB? For example, if a file is re-processed and its embeddings change, how should the old data be handled?
> if an document is ever upserted, the embedding needs to occur BEFORE insertion into arango, otherwise I think the vector index will allow insertion

Phase 7 (Error Handling and Logging):

Are there any specific requirements for the format or content of log messages?
> use Loguru and colors for log level. Include a ✅ All tests passed or X tests failed at the end of the logs so it's easy to get a report

Should the system generate summary reports or dashboards to track the overall progress and success rate of the pipeline?
> yes. use tabulate or some kind of table for metrics

Should logging levels be configurable via environment variables or a configuration file?
config file is fine

Phase 8 (Pipeline Orchestration):

Should the main orchestration script be responsible for managing the entire pipeline workflow, or should it delegate certain tasks to separate modules or services?
> yes. there should be a main orchestation script that that agent/humman uses by fastapi or the cli. the main orchestration script imports the modules it needs as steps in the pipeline. The script should be very easy to follow. Step 1, Step 2.

Phase 9 (CLI Implementation):

Should the CLI be able to accept multiple repository URLs as input, or should it only process one repository at a time?
> multiple (a list of repo urls) if possible and NOT overly complex
Should the CLI provide options for configuring the behavior of the pipeline (e.g., enabling/disabling caching, setting rate limits, specifying file exclusion patterns)?
> yes. the cli should allow the agent/human to easily filter or force 
Phase 10 (FastAPI):

Should the FastAPI expose authentication or authorization?
> use best practice here. Remember this is an MVP (authentication/authorization is probably too complex for MVP)

What additioanl clarifying questions do you have before you create the task.md plan file